...
public class NP3 {
    ...
    private static void readfile() {
        String path = "C:\\Users\\HoundAZ\\Documents\\Algor\\NP3\\src\\np3\\m2ii.txt";
    ...
    /**
     * ในเมตริกที่จะสร้างเป็นคำตอบ จะยาว 2*n + 3*m ช่อง โดย 2*n ช่องแรกจะเก็บ
     * vertex ที่เป็น type 3*m ช่องหลังจะเก็บ vertex ที่เป็น 3เหลี่ยม
     */
    private static void vertexCover() {
        ...
        // จัดการ vertex 2n ตัวแรก ต้องเชื่อมกันระหว่าง (1 -> -1) (2 -> -2) เป็นต้น
        for (int i = 0; i < 2 * n; i += 2) { // i will scan index of keep_type
            addEdge(i, i + 1);
        }

        // จัดการ vertex 3*m ตัวหลัง
        for (int i = 0; i < 3 * m; i++) { // i will scan index of keep_triAngle which size = 3m
            // สร้างสามเหลี่ยม ให้ทุกๆ 3 ช่องในอาเรย์คำตอบเชื่อมกันเอง
            if (i % 3 == 0) {
    ...
    /**
     * เมดตอดนี้จะเซต type ของข้อมูลว่ามี type อะไรบ้างไว้ใน keep_type
     */
